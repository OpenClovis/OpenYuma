                    ...
                    dlq_hdr_t *datadefQ)
{
    ...
    if (augextern || augdefcase) {
        ...
        for (testobj = (obj_template_t *)dlq_firstEntry(augQ);
            ...
            if (testobj->tkerr.mod->langver == NCX_YANG_VERSION11 && obj_is_mandatory(testobj)) {
                if (augextern) {
...
static status_t 
    get_resolved_xpath (tk_chain_t *tkc,
                   ncx_module_t *mod,
                   dlq_hdr_t *datadefQ, 
                   int *size, 
                   char** arrayxpath)
    ...
    for (testobj = (obj_template_t *)dlq_firstEntry(datadefQ);
		...
		if (!obj_has_name(testobj)) 
		{
			log_debug4("\n 3 resolve_xpath: %s", 
                ...
                xmlChar *mybuff = NULL;
                res = obj_gen_object_id(testobj, &mybuff);
                if (res == NO_ERR && obj_is_leaf(testobj)) {
					//if(*size == 0)
					//	arrayxpath = (char** )malloc(1024 * sizeof(char*));
                    arrayxpath[*size] = (char*) malloc(xml_strlen(mybuff)+1);
                    strcpy(arrayxpath[(*size)++], (const char*)mybuff);
                } else {
					
                    log_debug4("\n 2 resolve_xpath: %s", obj_get_name(testobj));
                }
                if (mybuff) {
                    m__free(mybuff);
                }
        ...
        case OBJ_TYP_LIST:
             ...
             */
             /*log_debug4( "\n THAI resolve_xpath: "
                                    "list in mod %s, object %s, on line %u",
                                    mod->name, obj_get_name(testobj), testobj->tkerr.linenum);
                                    * */
            if (is_targetmod) {
                for (key = obj_first_key(testobj);
            ...
            /* check list children */
            res = get_resolved_xpath(tkc, mod, testobj->def.list->datadefQ,
                                size,
                                arrayxpath);
            CHK_EXIT(res, retres);
            break;
        case OBJ_TYP_CHOICE:
            res = get_resolved_xpath(tkc, mod, testobj->def.choic->caseQ,
                                size,
                                arrayxpath);
            break;
        case OBJ_TYP_CASE:
            res = get_resolved_xpath(tkc, mod, testobj->def.cas->datadefQ,
                                size,
                                arrayxpath);
            break;
        case OBJ_TYP_RPC:
            res = get_resolved_xpath(tkc, mod, &testobj->def.rpc->datadefQ,
                                size,
                                arrayxpath);
            break;
        case OBJ_TYP_RPCIO:
            res = get_resolved_xpath(tkc, mod, &testobj->def.rpcio->datadefQ,
                                size,
                                arrayxpath);
            break;
        case OBJ_TYP_NOTIF:
            res = get_resolved_xpath(tkc, mod, &testobj->def.notif->datadefQ,
                                size,
                                arrayxpath);
            break;
        case OBJ_TYP_REFINE:
			    ...
			    dlq_hdr_t *datadefQ, int* size, char** xpaths)
{
    ...
    /* first resolve the main module or top submodule */
    res = get_resolved_xpath(tkc, mod, datadefQ, size, xpaths);
    CHK_EXIT(res, retres);
    ...
    for (node = (yang_node_t *)dlq_firstEntry(&mod->allincQ);
        ...
        for (node2 = (yang_node_t *)dlq_firstEntry(&mod->allincQ);
            ...
            res = get_resolved_xpath(tkc, 
                                node->submod,
                                &node2->submod->datadefQ, size, xpaths);
            CHK_EXIT(res, retres);
        }
    }
